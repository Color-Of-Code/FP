...
`F` must be an [Applicative](./12-applicative.md) so that independent effects can be combined. `T`
must be a [Functor](./11-functor.md) so that its shape can be mapped over. Many traversable types
are also [Foldable](./13-fold.md).
