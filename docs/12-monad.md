...
`fmap` (from [Functor](./08-functor.md)) handles `f :: a ⟶ b`. But when `f :: a ⟶ Mb` (the function
...
- **`pure :: a ⟶ Ma`** — lift a plain value into the monad (inherited from
  [Applicative](./09-applicative.md)). Also called `return`.
...
For combining multiple monads in one computation, see [13. Monad Transformers](./13-transformer.md).
