...
`F` must be an [Applicative](./05-applicative.md) so that independent effects can be combined. `T`
must be a [Functor](./04-functor.md) so that its shape can be mapped over. Many traversable types
are also [Foldable](./06-fold.md).
